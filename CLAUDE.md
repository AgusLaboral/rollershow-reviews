...
- **Composición desktop de producto**: el encabezado del ambiente ocupa su propia fila. Debajo, tarjeta de producto y uploader deben arrancar exactamente en la misma línea. No alinear el uploader con el título ni usar alturas mínimas para fabricar equilibrio.
...
- **Micro-recompensas causales, no gamificación genérica**: cada ganancia real responde en el lugar de la acción y confirma qué quedó guardado. Foto/video usa luz sobre la cortina y puntos hacia el contador; estrellas se ordenan; audio asienta su onda; texto cierra una línea. El copy rota para no sentirse mecánico. Sólo puede existir una recompensa global a la vez, no bloquea interacción ni foco y `aria-live` comunica el resultado. No agregar confetti, emojis, badges o sonidos a cada paso; la cortina sigue siendo el gesto principal.
