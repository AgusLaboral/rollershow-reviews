...
- **Cortina roller sunscreen, no telón teatral**: el cambio de paso debe tener un gesto inequívoco de producto. El mecanismo superior usa `img/roller-mechanism-real.png`, render de 1600 px del modelo completo convertido que entregó Agus, y permanece anclado arriba durante los pasos 1–7; se oculta en portada y cierre. La trama sunscreen nace 3 px detrás del tubo, comparte temperatura, escala de tejido y luz con él, baja translúcida, cubre y revela la etapa siguiente. No puede haber luz, hueco ni cambio de material entre mecanismo y tela. Evitar pliegues, terciopelo, mecanismos CSS inventados o teatralidad cursi. No usar `backdrop-filter` ni blur sobre los planos animados: en algunas GPU generan cuadros negros durante la composición.
...
- **Video de fondo terminado, no salida cruda del generador**: los clips Kling se entregan como ciclos ping-pong de 10,1 s a 48 fps, H.264 `faststart`, 1440×810 en desktop y 810×1440 en mobile. Los tres `<video>` usan `loop`, `muted` y `playsinline`; el último cuadro vuelve al primero mediante el tramo invertido, nunca con un corte directo. Los posters salen de los PNG maestros y las escenas posteriores se hidratan al entrar para no cargarlas en portada. No volver a publicar los MP4 crudos de 5 s/24 fps ni posters de 60 KB.
...
- **Composición desktop de producto**: el encabezado del ambiente ocupa su propia fila. Debajo hay una única superficie de producto a sangre, con velo sunscreen y CTA de carga integrado. En desktop el prompt ocupa aproximadamente las cinco columnas derechas, centrado verticalmente en su sector; `Seguir sin foto` queda inmediatamente debajo dentro de la misma decisión. En mobile ambos se apoyan abajo. No arrinconar el CTA ni recrear dos cajas enfrentadas.
...
- **El mecanismo persistente es producto, no decoración del puntaje**: junto a chances/puntos no vive ningún tubo CSS, cadena ni imitación 3D. El único mecanismo admitido es el asset real aprobado, full width, fijo en el aire entre encabezado y contenido durante los pasos 1–7. Funciona como origen físico de la transición y no como indicador de progreso. Debe conservar al menos 4 px de separación con el título y la tela debe salir detrás de su tubo; cualquier reconstrucción con gradientes CSS sigue prohibida.
...
- **Acciones posteriores unidas al producto**: cuando una foto o video reemplaza el placeholder, la superficie y su bandeja de acciones forman una sola pieza. En desktop el CTA puede conservar peso a la derecha, pero no flotar debajo de la grilla; la acción secundaria y la primaria comparten borde, fondo y ancho con la imagen.
