...
- **Video de fondo terminado, no salida cruda del generador**: los clips Kling se entregan como ciclos ping-pong de 10,1 s, H.264 `faststart`, 1440×810 a 48 fps en desktop y 720×1280 a 30 fps en mobile. Los tres `<video>` usan `loop`, `muted` y `playsinline`; el último cuadro vuelve al primero mediante el tramo invertido, nunca con un corte directo. Los posters salen de los PNG maestros y las escenas posteriores se hidratan al entrar para no cargarlas en portada. No volver a publicar los MP4 crudos de 5 s/24 fps ni exigir 48 fps en mobile si el decoder pierde cuadros.
