...
Web app multistep (mockup) para que clientes que **ya compraron** cortinas Rollershow suban fotos/video de las instaladas, califiquen con estrellas, dejen un audio (estilo nota de voz de WhatsApp) y una opinión escrita. Todo suma tickets que son chances directas de un sorteo mensual, anunciado en Instagram. Objetivo real de negocio: generar contenido (UGC) real para marketing, capitalizando clientes que ya confiaron, no leads sin convertir.
...
- **Tickets (reemplaza al sistema de puntos/chances, pedido por Agus 2026-07-20)**: cada aporte suma tickets directos con un ícono de ticket y un `+1` que vuela al contador: estrellas +1, texto +1, foto +1 c/u, video +1 c/u, audio +3. Cada ticket es una chance en el sorteo; toda participación confirmada tiene mínimo 1. Google duplica los tickets. No reintroducir puntos ni conversiones intermedias.
...
- **Celebración final intensa y honesta**: el último paso puede saturarse de recursos porque ya no enseña ni pide explorar; celebra el hito. La fiesta sale de los premios, la cortina sunscreen y los tickets reales. No inventar promedios, rankings ni cercanía determinista al premio; cada ticket se explica como una chance en el sorteo.
- **Gracias es una culminación, no una confirmación centrada**: después de participar, los tickets dejan de ser cards y se vuelven tipografía monumental (un único número con su ícono de ticket). El roller libera fibras con gravedad, profundidad e inercia; las partículas pasan por detrás de las zonas legibles. La secuencia termina transfiriendo el foco a Google. No volver al check circular, dos contadores en cajas y confetti DOM uniforme.
- **Audio en dos decisiones, nunca envío implícito**: mientras se graba, `Terminar y guardar` es el único CTA visible y pulsante. Guarda la captura local y suma sus tickets, pero no envía ni avanza; después aparecen escuchar, volver a grabar, borrar y `Continuar`. Al guardar u omitir, ese CTA normal entra antes del texto opcional, se centra en el viewport, recibe foco real y un énfasis transitorio; en desktop es compacto, no una barra de ancho completo. La frase de +1 ticket queda como complemento visualmente secundario debajo. `Seguir sin audio` no puede competir durante la grabación.
...
1. **Integración con la API real**: reemplazar el bloque `PRESUPUESTO` hardcodeado por `GET /api/v2/presupuesto/:token` (cliente, vendedor con foto real, ítems comprados) y el `enviar()` simulado por `POST /api/v2/reviews` (multipart: fotos/video/audio/estrellas/texto/consentimiento). **Los tickets deben recalcularse server-side siempre** — nunca confiar en el valor que manda el cliente.
...
- **Micro-recompensas causales, no gamificación genérica**: cada ganancia real responde en el lugar de la acción y confirma qué quedó guardado. Foto/video usa luz sobre la cortina; estrellas se ordenan; audio asienta su onda; texto cierra una línea. Después, un único ticket rojo con `+N` recorre una trayectoria fija desde esa acción hasta el contador estable y el contador recibe el impacto. No puede nacer en coordenadas aleatorias ni quedar flotando. El ticket muestra ganancia exacta y el burst el total nuevo; el copy rota y `aria-live` comunica el resultado. No agregar confetti, emojis, badges ni count-ups. Todo control habilitado que se presenta visualmente como botón —incluidos selectores de archivo y CTAs externos— responde a su activación con el mismo clic seco, original y casi imperceptible; links de texto, cambios de estado y transiciones automáticas permanecen en silencio. Una carga completada suma un chime corto y el cierre expande ese motivo al confirmar la participación.
...
- **Acciones posteriores unidas y centradas**: cuando una foto o video reemplaza el placeholder, la superficie y su bandeja forman una sola pieza. Primaria y secundaria comparten un eje central y un ancho máximo de 640 px; avanzar va arriba y agregar contenido debajo. No volver a repartirlas en columnas, alinear una a cada lado ni dejar un link huérfano. La secundaria usa `Agregar otra foto o video` y explicita en otra línea que cada archivo más suma otro ticket.
- **La recompensa acompaña la decisión**: toda acción que crea una recompensa anticipa el valor exacto en el propio CTA o pegado al control: foto/video `+1 ticket`, audio `+3 tickets`, calificación `+1 ticket`, texto `+1 ticket` y Google `duplicá tus tickets`. Esto aplica aunque el beneficio ya se haya explicado arriba. No prometer tickets en botones que sólo avanzan, reemplazan o revisan contenido.
- **Cierre desktop ordenado por estado**: el total de tickets es el único logro numérico; no reintroducir un segundo contador. No cruzar el bloque inferior con divisores u órbitas. El éxito de Google usa un check SVG de 48 px, texto de alto contraste y `role=status`; no un tilde suelto de 17 px.
...
- **Un total no se lee dos veces**: en confirmación, el logro grande reemplaza al contador compacto del encabezado. El roller sigue visible a todo el ancho, pero los números de arriba se retiran para no competir con el total de tickets protagonista.
...
## Reemplazo de referencia y recompensa — 2026-07-15
...
- Con varias cargas se muestra la última. Borrarla recupera la anterior; borrar la única recupera el placeholder, retira el estado completado y recalcula los tickets.
...
## Ronda de Nicolás — 2026-07-23

- **Premio genérico en la comunicación, detalle sólo en Bases**: los productos cambian mes a mes, así que portada, confirmación, Gracias y salida dicen `3 sets de textiles premium`. `SORTEO.premioGenerico` y `premioGenericoDetalle` alimentan ese copy; `SORTEO.premios` sigue existiendo pero su detalle por ganador se publica únicamente en las Bases, que es donde el Decreto 961/2017 lo exige. No volver a nombrar mantas, almohadones ni alfombras en el recorrido.
- **Consentimiento corto y anónimo**: el CTA dice `Autorizar y participar`, en desktop es compacto (`width:auto`, no barra a sangre) y arranca en la misma línea que el bloque de tickets (`align-self:start`, no `center`: con `center` quedaba 33 px más abajo y se leía desalineado). Debajo, alineada al mismo eje izquierdo, la letra chica explica que el material se usa sólo para mostrarle a otros clientes cómo quedan las cortinas y que la publicación es anónima, sin nombre ni datos. No volver al texto largo dentro del botón ni a la nota centrada.
- **Google abre a calificar, no a leer opiniones**: el CTA usa el link corto oficial `https://g.page/r/CWgiNVZ7UVRD/review` (el mismo formato de los QR de locales, deriva del CID `0x4354517b56352268`). Equivalente para web: `search.google.com/local/writereview?placeid=ChIJa1mZkHG2vJURaCI1VntRVEM`. No volver a un `maps/place/...` que abre la ficha.
- **Footer mínimo**: sólo `Rollershow` y `Bases y condiciones`.
- **Fondo en video: ritmo antes que fidelidad**. Los clips de portada se reencodean para decodificación barata (`main@3.1`, `-tune fastdecode`, sin CABAC ni B-frames, GOP corto, `faststart`); el póster se extrae del cuadro cero del propio encode. La capa se promueve un cuadro antes de reproducir —componer y decodificar juntos descartaba cuadros en la entrada, y reproducir con la capa invisible los descarta igual porque el navegador no pinta lo que no se ve—. El arranque espera `canplaythrough` con fallback por `canplay`, y el reinicio del loop ocurre durante el fundido, no en `ended` con el video ya visible. En equipos de gama baja o red lenta (`saveData`, 2g/3g, ≤4 GB y ≤4 núcleos) no se reproduce nada: queda el póster quieto, decidido por un script inline junto al propio `<video>` para poder abortar la descarga. Se verifica con `scripts/verify-video-pacing.mjs`.
