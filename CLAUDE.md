...
- **Cierre en dos estados**: antes del consentimiento, la autorización completa es el único CTA rojo y `Confirmar y participar` permanece visible pero desactivado. Después de autorizar, el consentimiento baja de volumen y confirmar toma el color, el pulso y el foco visual. Nunca hay dos CTAs activos compitiendo.
...
- **Roller persistente del puntaje**: debajo de chances/puntos vive un único cilindro gris con volumen leve, no una línea divisoria de punta a punta. En cada cambio de etapa ese mismo cilindro se despliega sobre el grid y libera la tela sunscreen hacia abajo; reposo y transición deben leerse como el mismo objeto.
