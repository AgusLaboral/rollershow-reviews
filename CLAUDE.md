...
- **Paños finales sunscreen, no placas opacas**: los cinco paños de `.gr-curtain-reveal` transmiten el ambiente con una base de alfa baja y conservan malla bidireccional, luz irregular y fibra. No usar color sólido, blur ni `backdrop-filter`: el material filtra luz y deja sobrevivir siluetas, no se comporta como vidrio frost. La regresión exige alfa máximo de .35 y textura verificable.
- **Video de fondo terminado, no salida cruda ni duración fingida**: los clips Kling son tomas continuas de 15 s con desplazamiento físico de cámara —arco, paneo y tilt con paralaje—, no zoom digital ni ping-pong. Se entregan H.264 `faststart` en la resolución nativa útil —1280×720 en desktop y 720×1280 en mobile— a 24 fps; no hacer upscale que agregue decodificación sin sumar detalle. Usan `muted` y `playsinline`, sin `loop`. Al terminar quedan quietos en el último encuadre y sólo reinician cuando el usuario vuelve a entrar a esa etapa. Los posters salen de los PNG maestros y las escenas posteriores se hidratan al entrar para no cargarlas en portada. La aceptación exige leer producto, textura, horizonte y recorrido en contactos temporales, además de medir duración, cuadros perdidos y cierre en el navegador.
...
- **Acciones posteriores unidas y centradas**: cuando una foto o video reemplaza el placeholder, la superficie y su bandeja forman una sola pieza. Primaria y secundaria comparten un eje central y un ancho máximo de 640 px; avanzar va arriba y agregar contenido debajo. No volver a repartirlas en columnas, alinear una a cada lado ni dejar un link huérfano. La secundaria usa `Agregar más contenido` y explicita en otra línea `Foto +10 puntos. Video +25 puntos.`
